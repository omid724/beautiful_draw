...
def draw_world_1(r_main=75, r_relative=55, turn=300):
    ...
    plt.text(-total_dist - 20, -total_dist - 5, f"color = {color}", fontdict=font)
    for i in range(turn):
        theta = np.linspace(i * 2 * np.pi, (i + 1) * 2 * np.pi, 600)
    ...
    plt.savefig(f"output/figure{time_stamp}.png", dpi=600, bbox_inches="tight")
    plt.clf()
